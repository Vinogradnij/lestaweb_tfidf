...
2. В файле .env-template, при необходимости, заменить стандартные
адрес и порт хоста для запуска приложения (TFIDF__RUN__)
3. Создать образ для докера: в корневом каталоге проекта выполнить 
команду *docker build . -t app:1.1.0*
## Запуск
* Выполнить команду *docker run -p 5050:8000 -it app:1.1.0*
* Сервис будет доступен по адресу 127.0.0.1:5050