...
* Метрики:
  * Количество обработанных коллекций / алгоритма Хаффмана
  * Минимальное время обработки коллекции / алгоритма Хаффмана
  * Среднее время обработки коллекции / алгоритма Хаффмана
  * Максимальное время обработки коллекции / алгоритма Хаффмана
  * Общее время, потраченное на обработку коллекций / алгоритма Хаффмана
  * Время обработки последней коллекции в формате unix timestamp
  * Время обработки последнего запроса на кодирование алгоритмом Хаффмана
## Использование
### Версия приложения - 1.5.2
### Описание API
...
2. Необходимо скачать из репозитория:
   ...
   * .env, команда:  *curl -o .env https://raw.githubusercontent.com/Vinogradnij/lestaweb_tfidf/refs/heads/master/.env-template*
...
* huffman/
  * priority_queue.py - содержит реализацию приоритетной очереди
  * service.py - содержит вспомогательные функции для работы с алгоритмом Хаффмана
  * tree.py - содержит классы, реализующие сущности Узел и Дерево Хаффмана
* info/ - директория предназначенная для работы со служебной информацией приложения
  * crud.py - бизнес логика обработки путей
  * models.py - модель данных для Метрик
